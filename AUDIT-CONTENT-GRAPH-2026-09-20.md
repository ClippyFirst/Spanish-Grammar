# Content graph audit — 2026-09-20

Pages: 195
Nodes with related: 195
Markdown internal links: 590
Broken targets: 0
Ambiguous targets: 0
Self-links: 0
Duplicate related entries: 0
Semantic orphans (no inbound and not category-reachable): 0
Category-index reachable: 195
Navigation orphans (not reachable from category index): 0
Category-only leaf pages (no semantic inbound): 50
Manual related sections: 171
Manual/automatic related duplicates: 0

## Semantic orphans (not category-reachable)

## Navigation orphans

## Category-only leaf pages (no semantic inbound)
- adjectives/adjective-complements,
- adjectives/adjective-types-cultos,
- adjectives/agreement-traps,
- adjectives/meaning-by-position,
- adverbs/focus-and-stance,
- articles/articles-with-names,
- conjunctions/como-excepto,
- conjunctions/discourse-fillers,
- conjunctions/que-de-que,
- fundamentals/grammar-basics,
- fundamentals/numerals-advanced,
- micro-constructions/de-inf-sera-no-sea-lo-de,
- moods/free-subjuntivo,
- moods/futuro-de-subjuntivo,
- moods/perfecto-subjuntivo,
- moods/quizas-tal-vez-acaso,
- moods/subjunctive-triggers-list,
- nouns/irregular-plurals,
- nouns/possessive-constructions,
- periphrases/estar-a-punto-de,
- periphrases/overview,
- prepositions/estar-de-ser-de,
- prepositions/place-review,
- prepositions/verb-preposition-pairs,
- pronouns/demonstrative-pronouns,
- pronouns/exclamatives,
- pronouns/mismo-uno-generic,
- pronouns/reciprocal-pronouns,
- regional/andean,
- regional/caribbean,
- regional/colombia-canarias,
- regional/leismo-laismo-loismo,
- regional/rioplatense,
- sentence-structure/absolute-constructions,
- sentence-structure/interjections,
- sentence-structure/modality-speech-acts,
- sentence-structure/personal-a,
- sentence-structure/reported-speech-review,
- sentence-structure/si-clauses-overview,
- sentence-structure/verum-focus,
- sentence-structure/word-order-review,
- spelling/h-y-ll,
- tenses/preterito-anterior,
- verbs/change-of-state,
- verbs/deber-vs-deber-de,
- verbs/haber-vs-tener,
- verbs/infinitive-as-noun,
- word-formation/adverb-diminutives,
- word-formation/compounding,
- word-formation/diminutives-review,

## Broken Markdown links

## Ambiguous targets

## Self-links

## Duplicate related entries

## Manual/automatic related duplicates

## Overlinked pages (12+ outgoing)
- fundamentals/ua-interference: 24

## Graph policy
Preferred direction: quick reference → primary explanation → deep dive → adjacent/specialized topic.

Category indexes are first-class navigation hubs; a page with no semantic inbound is not an orphan if it is listed by its category index.
Review/overview pages should navigate to canonical pages, not duplicate them.
related is for genuinely useful neighboring topics, not every page in the same category.
Remove self-links, duplicates, broken targets and ambiguous basename targets.
Do not delete unique grammar coverage merely to reduce graph size.
