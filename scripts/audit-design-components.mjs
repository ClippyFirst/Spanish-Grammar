...
for(const b of [...src.matchAll(/<button\b[\s\S]*?<\/button>/g)].map(m=>m[0])){const opening=b.match(/^<button\b[\s\S]*?>/i)?.[0]??'';const body=b.replace(opening,'').replace(/<\/button>\s*$/i,'').trim();if(!/aria-label=|aria-labelledby=|title=/.test(opening)&&!body.replace(/<[^>]+>/g,'').trim())add('HIGH','accessibility','Possibly unnamed button',b.replace(/\s+/g,' ').slice(0,160),'Give icon-only buttons an accessible name.');}
...
for(const p of [['#5f5d56','#f3eee4'],['#a83b2a','#f3eee4'],['#7f2f24','#f3eee4'],['#17636c','#f3eee4'],['#d0cbc0','#171815'],['#b8b2a6','#171815'],['#77c2c8','#171815']]){if(css.includes(p[0])&&css.includes(p[1])){const ratio=cr(p[0],p[1]);if(ratio<4.5)add('HIGH','color','Potential AA contrast issue: '+p[0]+' on '+p[1],'Estimated ratio '+ratio.toFixed(2)+':1.','Verify in a rendered contrast checker and adjust semantic tokens if needed.');}}

const out=path.join(R,'docs','audits','current','design-deep.md');
