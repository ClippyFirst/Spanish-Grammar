...
for(const b of [...src.matchAll(/<button\b[\s\S]*?<\/button>/g)].map(m=>m[0]))if(!/aria-label=|aria-labelledby=|title=/.test(b)&&!/>\s*[^<]+\s*<\/button>/.test(b))add('HIGH','accessibility','Possibly unnamed button',b.replace(/\s+/g,' ').slice(0,160),'Give icon-only buttons an accessible name.');
